...
//I added the following flag to turn part of the drawing procedure off to simplify the kin term map
   ...
   public void registerListeners() {
	...
	public KintermEntry()
	{
		...
		//{{INIT_CONTROLS
		setSize(ScaleManager.s(278),ScaleManager.s(56));
		//}}
	   ...
	   registerListeners();
		...
		for(int i = 0;i< newHooks.length;i++) newHooks[i] = new Rectangle(0,0,ScaleManager.s(HHEIGHT),ScaleManager.s(HWIDTH));
	}
...
Color greenL = new Color(0,225,0,150);
Color greenR = new Color(0,178,0);
...
Color orangeL = new Color(220,178,0);
	...
	void drawProducts(Graphics g) { // override for other methods for drawing products and hooks
	   ...
	   for(int i=0;i<products.size();i++) {
		  ...
		  g.setColor(setRectColor(genColour,arrowType));//color of rects
			 if (gsex != 0) {
				if (p.getOrientation() == SPOUSE || p.getOrientation() == SPOUSE+1) {
				   ...
				   if (drawFlag)
					  g.fillRect(r.x,r.y,r.width-ScaleManager.s(5),r.height-ScaleManager.s(5));
				}
			 }
		...
		!(p.getOrientation() == SPOUSE || p.getOrientation() == SPOUSE+1)) { // what about my sex == neutral?????????
				...
				startp.translate(getLocation().x+ScaleManager.s(HWIDTH)/2,getLocation().y+ScaleManager.s(HHEIGHT)/2);
	...
	// addProduct, where I have put support for this.
				   ...
				   tohook.translate(ke.getLocation().x+ScaleManager.s(HWIDTH)/2,ke.getLocation().y+ScaleManager.s(HHEIGHT)/2);
...
*/
	...
	*/
	 void setRectangles(Graphics g) {
    	...
    	int vOffset = ScaleManager.s(12);
    	int hOffset = ScaleManager.s(12);

     	if ((hOffset+hOffset+swid+1) < (5*ScaleManager.s(HWIDTH)+ScaleManager.s(14))) swid += (5*ScaleManager.s(HWIDTH)+ScaleManager.s(14)) -( hOffset+hOffset+swid+1);
		...
		setSize(new Dimension(hOffset+hOffset+swid+1,sht+vOffset+vOffset+2));
  		...
  		int j = 0;
  		int wido = ScaleManager.s(HWIDTH)/2;
  		int hto = ScaleManager.s(HHEIGHT)/2;
  		int off=0;

  		for (int i=0;i<3;i++,off = i * NOFFSET) {
  			newHooks[j+off].setLocation(wid/2+(i-1)*ScaleManager.s(14)-wido,i == 1 ? ScaleManager.s(2) : ScaleManager.s(6));
  		}
  		j=1;off=0;
   		for (int i=0;i<3;i++,off = i * NOFFSET) {
  			newHooks[j+off].setLocation(wid/2+(2 - i-1)*ScaleManager.s(14)-wido,ht-ScaleManager.s(HHEIGHT)- (i == 1 ? ScaleManager.s(3) : ScaleManager.s(2)));
  		}
  		j=2;off=0;
    	for (int i=0;i<3;i++,off = i * NOFFSET) {
  			newHooks[j+off].setLocation(wid-ScaleManager.s(11),ht/2+(i-1)*ScaleManager.s(HHEIGHT)-hto);
  		}
    	j=3;off=0;
    	for (int i=0;i<3;i++,off = i * NOFFSET) {
  			newHooks[j+off].setLocation(0,ht/2+(2 -i-1)*ScaleManager.s(HHEIGHT)-hto);
  		}
   		j=4;off=0;
     	for (int i=0;i<3;i++,off = i * NOFFSET) {
		...
		topHook.setBounds(wid/2-ScaleManager.s(5),0,ScaleManager.s(10),ScaleManager.s(10));
 		bottomHook.setBounds(wid/2-ScaleManager.s(5),ht-ScaleManager.s(12),ScaleManager.s(10),ScaleManager.s(10));
 		rightHook.setBounds(wid-ScaleManager.s(11),ht/2-ScaleManager.s(5),ScaleManager.s(10),ScaleManager.s(10));
 		leftHook.setBounds(0,ht/2-ScaleManager.s(5),ScaleManager.s(10),ScaleManager.s(10));
		spouseHook.setBounds(0,0,ScaleManager.s(10),ScaleManager.s(10));
		spouseHookR.setBounds(0,0,ScaleManager.s(10),ScaleManager.s(10));
}
...
//	protected Font itsSmallFont = new Font("Helvetica",Font.BOLD,10);
	...
	*/
	protected Font itsFont = ScaleManager.font("Times",Font.PLAIN,12);
	...
	*/
	...
	void KintermEntry_MouseDragged(java.awt.event.MouseEvent event)
	{
		...
		if ((hook = whatHook(sx,sy)) != NONE) {
			...
			if (lex != -10000) {
				g.drawLine(lgx, lgy, lex, ley);
				g.fillRect(lex- ScaleManager.s(5),ley-ScaleManager.s(5),ScaleManager.s(10),ScaleManager.s(10));
			}
...
//			sy = theHooks[hook].getLocation().translate(getLocation().x,getLocation().y).y;
			...
			g.fillRect(ex- ScaleManager.s(5),ey-ScaleManager.s(5),ScaleManager.s(10),ScaleManager.s(10));
