...
* generator
 * 7/3/09 DR modified inArrowsSimilar to allow for side generators to be structurally similar
 * DR added utility procedure left_rightSide to test if pair of terms are generators with left,right orientation
 * modified isArrowSimilar to allow for side generators to be structurally simiiar
*/
...
//System.out.println("Effective products for :"+" k="+k);
      StringVector ret = new StringVector();
      TransferProduct p;
      try {
      ...
      String b;
		 //System.out.println("Effective products for :"+p+" k="+k+" gen "+gen+" genX="+lookupTerm(gen).getEffectiveTerm());
...
//System.out.println("k= "+k+"effective "+k.getEffectiveTerm());
		 if (k == null) System.out.println("k=null m="+m);

		 if ( ret.indexOf((b = k.getEffectiveTerm()))  == -1) ret.addElement(b);
	  }
      return ret;
    }
...
*/
    ...
    * and two in some cases where gendered (as in Trobriand terminology)
...
//+" sr "+spouser);
   ...
   */
   ...
   public boolean inArrowsSimilarXX(TransferKinInfo k1, TransferKinInfo k2) {
	  // test to see if the pattern if 'in' arrows is similar
	  ...
	  for (s1.reset();s1.isNext();) {
		 ...
		 for (s2.reset();s2.isNext();) {
			...
			if (h2.equals(g2)) {
			   ...
			   if (o1 != lookupTerm(h3).getOrientation()) {
				  Debug.prout(0,"inArrowsSimilar: orientation of arrows in for "+k1.getTerm()+" not similar to "+k2.getTerm());
				  return false;
			   }
			   found = true;
			}
	  ...
	  return true; // all in terms matched in name and arrow orientation of generator linking
   }

	public boolean left_rightSide(TransferKinInfo k1, TransferKinInfo k2) {
		if (k1.isGenerator() && k2.isGenerator() && 
			(((k1.getOrientation() == TransferKinInfo.LEFT) && (k2.getOrientation() == TransferKinInfo.RIGHT))
			 ||((k1.getOrientation() == TransferKinInfo.RIGHT) && (k2.getOrientation() == TransferKinInfo.LEFT)))) {
			return true;
		}
		else return false;
	}
	...
	public boolean inArrowsSimilar(TransferKinInfo k1, TransferKinInfo k2) {
		...
		StringVector s2 = getInverse(k2.getTerm());
		String g0 = s2.toString(); // for referencing
		String gg0 = s1.toString();
		boolean lrFlag = left_rightSide(k1, k2);
		if (s1.size() != s2.size()) return false;//not similar
		int i = -1;
		for (s1.reset();s1.isNext();) {
			i++;
			String g1 = s1.getNext();
			String gg1 = (String)s2.elementAt(i);
			int marker;
			String g2 = g1.substring(0,(marker = g1.indexOf(';'))); // term
			String g3 = g1.substring(marker+1); // generator
			String gg2 = gg1.substring(0,(marker = gg1.indexOf(';'))); // term1
			String gg3 = gg1.substring(marker+1); // generator1
			if (g0.indexOf(g2+";") == -1) { // is term present? if not bail out
				if ((lrFlag) && g2.equals(g3) && gg2.equals(gg3)) {} //term;term and term1;term1 ok for side generators
				else {
					Debug.prout(0,"inArrowsSimilar: arrows in for "+k1.getTerm()+" not similar to "+k2.getTerm());
					return false;
				}
			...
			for (s2.reset();s2.isNext();) {
				String h1 = s2.getNext();
				String h2 = h1.substring(0,(marker = h1.indexOf(';'))); // term
				if (lrFlag && g2.equals(g3) && gg2.equals(gg3)){// side1;side1, side2;side2 are consistent with similarity
					found = true;
				}
				else if (h2.equals(g2)) {
					String h3 = h1.substring(marker+1); // generator
					int o2 = lookupTerm(h3).getOrientation();
					if (o1 != o2) {
						if (((o1 == k1.LEFT) && (o2 == k2.RIGHT)) || ((o1 == k1.RIGHT) && (o2 == k2.LEFT))) {}//left,right ok
						else {
							Debug.prout(0,"inArrowsSimilar: orientation of arrows in for "+k1.getTerm()+" not similar to "+k2.getTerm());
							return false;
						}
...
//DR added this procedure
    public ListVector findStructuralSimilar() {
	    ...
	    for (int i=0;i<size()-1;i++) {
		    ...
		    for (int j=i+1;j<size();j++) {
				...
				if (left_rightSide(a,b)) {
					if (!_sideGeneratorsSimilar(a, b)) continue; //do not merge			
				}
				else if (!a.outArrowsIdentical(b)) continue; // do not merge
	...
	public boolean _sideGeneratorsSimilar(TransferKinInfo k1, TransferKinInfo k2) {
		TransferProductsVector p1 = k1.getProducts();//compare [side1],[side2,side2] to [side1,side1],[side2]
		TransferProductsVector p2 = k2.getProducts();//or [side1,I],[side2,side2] to [side1,side1],[side2,I]
		TransferProduct tp1 = null;
		TransferProduct tp2 = null;
		TransferProduct tp11 = null;
		TransferProduct tp22 = null;
		String term1 = k1.getTerm();
		String term2 = k2.getTerm();
		if (p1.size() != p2.size()) return false;
		for (int i=0;i<p1.size();i++){
			tp1 = (TransferProduct)p1.elementAt(i);
			tp2 = (TransferProduct)p2.elementAt(i);
			Boolean flag1 = false;
			Boolean flag2 = false;
			if (!tp1.equals(tp2) && i+1 < p1.size()) {
				tp11 =(TransferProduct) p1.elementAt(i+1);
				tp22 = (TransferProduct)p2.elementAt(i+1);
				String gen1 = tp1.getGenerator();
				String gen11 = tp11.getGenerator();
				String gen2 = tp2.getGenerator();
				String gen22 = tp22.getGenerator();
				if (tp1.size() == tp22.size() && tp1.size() == 1){
					if (term1.equals(gen1) && term2.equals(gen22) || term1.equals(gen22) && term2.equals(gen1)) {flag1 = true;}
					}
				if (tp11.size() == tp2.size() && tp11.size() == 1) {
					if (term1.equals(gen11) && term2.equals(gen2) || term1.equals(gen2) && term2.equals(gen11)) {flag1 = true;}
				}
				if (tp1.size() == tp22.size() && tp1.size() == 2){
					if (term1.equals(gen1) && term2.equals(gen22) || term1.equals(gen22) && term2.equals(gen1)) {
						if (tp1.getSymbol(0).equals(tp22.getSymbol(0))) {flag2 = true;}
						else if (tp1.getSymbol(0).equals(gen1) && (tp22.getSymbol(0).equals(gen22))) {flag2 = true;}
					}
				}
				if (tp11.size() == tp2.size() && tp11.size() == 2) {
					if (term1.equals(gen11) && term2.equals(gen2) || term1.equals(gen2) && term2.equals(gen11)) {
						if (tp11.getSymbol(0).equals(tp2.getSymbol(0))) {flag2 = true;}
						else if (tp11.getSymbol(0).equals(gen11) && (tp2.getSymbol(0).equals(gen2))) {flag2 = true;}
					}
				}
				if (flag1 && flag2) {
					i++;
			...
			else if (!tp1.equals(tp2)) return false;
		}
		return true;
	}
