...
public class PreferencesDialog extends Dialog
{
	...
	public PreferencesDialog(Frame parent, Preferences prefs, boolean modal)
	{
		...
		//{{INIT_CONTROLS
		setVisible(false);
		setSize(ScaleManager.s(600),ScaleManager.s(600));
	...
	//	setLayout(new GridLayout(x,1));
		...
		for (int i=0;i<prefkeys.size();i++) {
			...
			if (key.equals("Display_Scale_Factor")) {
				scaleChoice = new Choice();
				scaleChoice.add("1");
				scaleChoice.add("2");
				scaleChoice.add("4");
				String cur = prefs.getString(key);
				if (cur.equals("2")) scaleChoice.select("2");
				else if (cur.equals("4")) scaleChoice.select("4");
				else scaleChoice.select("1");
				Label note = new Label("(restart required)");
				p.add(l);p.add(scaleChoice);p.add(note);add(p);
			} else {
...
*/
	...
	void okButton_ActionPerformed(java.awt.event.ActionEvent event)
	{
		...
		for (int i = 0; i < components.length-1; i++)
		{
			...
			Component input = c.getComponent(1);
			if (input instanceof Choice) {
				prefs.put(g, ((Choice) input).getSelectedItem());
			} else if (input instanceof TextField) {
				prefs.put(g, ((TextField) input).getText());
			}
		}
